...
const Airdrop: React.FC = () => {
  ...
  // State for Pact.fi farms data
  const [pactFarmsData, setPactFarmsData] = useState<any[]>([]);
  ...
  // Function to fetch farms data from Pact.fi
  const fetchPactFarmsData = async () => {
    setIsLoadingPactFarms(true);
    setPactFarmsError(null);
    try {
      const response = await fetch(
        "https://api.pact.fi/api/farms/all?ordering=-tvl_usd"
      );
      if (response.ok) {
        const data = await response.json();
        console.log("Pact.fi Farms API response:", data);
        console.log("Pact.fi farms count:", data.length || 0);

        if (data && data.length > 0) {
          // Log first few farms for debugging
          data.slice(0, 3).forEach((farm, index) => {
            console.log(`Farm ${index + 1}:`, {
              on_chain_id: farm.on_chain_id,
              pool: farm.pool,
              apr: farm.apr,
              average_apr: farm.average_apr,
              tvl_usd: farm.tvl_usd,
              peak_tvl_usd: farm.peak_tvl_usd,
              displayed_ends_at: farm.displayed_ends_at,
            });
          });
        }

        setPactFarmsData(
          (data || []).filter((farm: any) => farm.programs.length > 0)
        );
      } else {
        console.error(
          "Pact.fi Farms API response not ok:",
          response.status,
          response.statusText
        );
        setPactFarmsError(`HTTP ${response.status}: ${response.statusText}`);
      }
    } catch (error) {
      console.error("Error fetching Pact.fi farms data:", error);
      setPactFarmsError(
        error instanceof Error
          ? error.message
          : "Failed to fetch Pact.fi farms data"
      );
    } finally {
      setIsLoadingPactFarms(false);
    }
  };
        ...
        try {
          ...
          if (powAsset && pairedAsset) {
            ...
            normalizedPairs.push({
              ...
              apr:
                apr +
                (pairedAsset.unit_name === "ALGO" ? 3.24 / 2 : 0) +
                (pactFarmsData
                  .find((farm: any) => farm.pool === pool.on_chain_id)
                  ?.programs?.reduce(
