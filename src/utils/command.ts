...
export const propose: any = async (options: ProposeOptions) => {
  ...
  const sk: any = options.sk || sks.deployer;
  const acc = { addr, sk };
  ...
  const proposeR = await ci.propose(
    new Uint8Array(Buffer.from(padStringWithZeroBytes(options.title, 64))),
    new Uint8Array(
      Buffer.from(padStringWithZeroBytes(options.description, 512))
    ),
...
export const castVote: any = async (options: CastVoteOptions) => {
  ...
  const castVoteR = await ci.cast_vote(
    new Uint8Array(Buffer.from(options.proposalNode, "base64")),
    options.support
  );
  if (options.debug) {
...
export const getProposalNode: any = async (options: GetProposalNodeOptions) => {
  ...
  const getProposalNodeR = await ci.get_proposal_node(
    new Uint8Array(Buffer.from(options.title, "base64")),
    new Uint8Array(Buffer.from(options.description, "base64"))
  );
  if (options.debug) {
...
export const decodeProposal = (proposal: any): Proposal => {
  return {
    ...
    proposer: proposal[2],
    proposalTitle: stripTrailingZeroBytes(
      Buffer.from(proposal[3]).toString("utf-8")
    ),
    proposalDescription: stripTrailingZeroBytes(
      Buffer.from(proposal[4]).toString("utf-8")
    ),
    proposalNode: Buffer.from(proposal[5]).toString("base64"),
    ...
    proposalActionHash: Buffer.from(proposal[14]).toString("base64"),
...
export const decodeVoter = (voter: any): Voter => {
  ...
  return {
    ...
    lastProposalNode: Buffer.from(voter[5]).toString("base64"),
  };
};
...
export const lockPower: any = async (options: LockPowerOptions) => {
  ...
  const buildN = [];
  {
    ...
    buildN.push({ ...txnO, payment: 1e5 }); // est payment
  }
  {
    const txnO = (
